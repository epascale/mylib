...
int  not_doublematrix(PyArrayObject *mat)  {
	if (PyArray_TYPE(mat) != NPY_FLOAT64 || PyArray_NDIM(mat) != 2)  {
...
int  not_boolmatrix(PyArrayObject *mat)  {
	if (PyArray_TYPE(mat) != NPY_UINT8 || PyArray_NDIM(mat) != 2)  {
		PyErr_SetString(PyExc_ValueError,
			"In not_boolmatrix: array must be of type uint8 and 2 dimensional (n x m).");
		return 1;  }
	return 0;
}
